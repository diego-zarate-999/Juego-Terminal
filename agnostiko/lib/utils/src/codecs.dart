...
extension BigIntToBytes on BigInt {
  /// Retorna una lista de bytes de 8 bits que representan este número.
  ///
  /// Ejemplo: si se tiene un BigInt con valor númerico hexadecimal de
  /// '0x3DCF89', lo cual equivale a '4050825' en decimal, esta
  /// transformación resultaría en la siguiente lista de bytes:
  /// ( 0x3D, 0xCF, 0x89 ).
  ///
  /// Falla con [FormatException] si el valor a encodear es negativo.
  Uint8List toHexBytes() {
    if (this < BigInt.zero) {
      throw const FormatException("can't encode negative value");
    }
    final str = this.toRadixString(16);
    return str.length.isOdd
        ? str.padLeft(str.length + 1, '0').toHexBytes()
        : str.toHexBytes();
  }
}

extension BytesToBigInt on Uint8List {
  /// Retorna un BigInt de la representación base16 de esta lista de bytes.
  ///
  /// Ejemplo: si se tiene una lista de bytes: ( 0x3D, 0xCF, 0x89 ), la
  /// transformación daría como resultado un BigInt con valor númerico
  /// hexadecimal de '0x3DCF89', lo cual equivale a '4050825' en decimal.
  BigInt toBigInt() {
...
Uint8List xorBytes(Uint8List first, Uint8List second) {
  if (first.length != second.length) {
    throw const FormatException("lists length don't match");
  }
  int bytesLen = first.length;
...
extension Xor on Uint8List {
  Uint8List xor(Uint8List other) {
    return xorBytes(this, other);
  }
}

/// Genera una lista de bytes padeada por la derecha a un tamaño de bloque fijo
Uint8List padBytesBlockRight(Uint8List data, int blockSize, int padding) {
  if (blockSize <= 0) {
    throw const FormatException("invalid block size");
  }
  final div = data.length ~/ blockSize;
  final remainder = data.length % blockSize;
  final paddedSize = remainder > 0 ? (div + 1) * blockSize : data.length;
  final paddedData = Uint8List(paddedSize);
...
/// Genera una lista de bytes padeada por la izquierda a un tamaño de bloque fijo
Uint8List padBytesBlockLeft(Uint8List data, int blockSize, int padding) {
  if (blockSize <= 0) {
    throw const FormatException("invalid block size");
  }
  final div = data.length ~/ blockSize;
  ...
  final startPosition = paddedSize - data.length;
  for (int i = startPosition; i < paddedSize; i++) {
    paddedData[i] = data[i - startPosition];
  }
  return paddedData;
}

extension PadBlock on Uint8List {
  /// Retorna los bytes padeados por la derecha a un tamaño de bloque fijo
  Uint8List padBlockRight(int blockSize, int padding) {
    return padBytesBlockRight(this, blockSize, padding);
  }

  /// Retorna los bytes padeados por la izquierda a un tamaño de bloque fijo
  Uint8List padBlockLeft(int blockSize, int padding) {
    return padBytesBlockLeft(this, blockSize, padding);
  }
}
