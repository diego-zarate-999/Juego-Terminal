...
class IsoMessage {
  ...
  /// Setea el valor del campo con identificador [fieldNumber].
  ///
  ...
  /// Setea el valor del campo binario [fieldNumber] desde una lista de [bytes].
  ///
  ...
  static void _setFieldsFromDataBytes(
    ...
    for (final fieldNum in fieldList) {
      ...
      if (fieldDef.isFixedLen) {
        ...
        if (fieldPacker.usesBytesLen) {
          fieldEnd = fieldStart + len;
        } else {
          fieldEnd = fieldStart + fieldPacker.packedLen(len);
        }
      }
  ...
  Uint8List get _packedDataElements {
    ...
    for (final key in sortedKeys) {
      ...
      if (fieldDef.isFixedLen) {
        ...
        final int len;
        if (fieldPacker.usesBytesLen) {
          len = fieldBytes.lengthInBytes;
        } else {
          len = _dataElements[key]?.length ?? 0;
        }
        final lenStr =
            len.toString().padLeft(fieldDef.fieldLenFormat.index, '0');
